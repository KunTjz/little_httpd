...
void on_worker_breakdown(int signo)
{
	...
	syslog(LOG_INFO, "master:%d receive sigchld", getpid());

	if((pid = waitpid(-1, &status, WNOHANG)) < 0) {
		printf("waitpid fail");
		return;
	}
	//todo:restart a child
	if(reset_worker(pid) == L_HTTP_FAIL) {
		syslog(LOG_INFO, "reset worker fail");
		exit(1);
	}	

	//syslog(LOG_INFO, "proc:%d breakdown", pid);
	//sleep(10);
	syslog(LOG_INFO, "master weak up");
}
